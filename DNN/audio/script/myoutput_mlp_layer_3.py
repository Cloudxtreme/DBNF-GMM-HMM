...
if __name__ == "__main__":
    ...
    L2=OutputLayer(numpy.load(mlpdirname+"/L2.W.npy"),
                    numpy.load(mlpdirname+"/L2.B.npy"))

    # ニューラルネットワークの出力を計算する数式の作成
    x=T.fmatrix("x")
    p=L2.forward(L1.forward(x))
    ...
    for line in open(filelist, 'r').readlines() :
        ...
        # 入力データ・出力ファイルの読み込み
        data=numpy.fromfile(indirname+'/'+line+'.mfc429_norm',dtype=">f4")
        data=data.reshape(len(data)/L1.inum,L1.inum)
        f=open(outdirname+'/'+line+'.dbnf',"wb") # 出力ファイルの名前
       ...
       # HTK形式の特徴量とするためのヘッダー部分の書き込み
        ...
        f.write(pack('>H',9))           # 特徴量の種類(オリジナルは9)
